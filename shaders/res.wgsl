...
@group(0) @binding(0)
var<uniform> u_mpv_matrix: mat4x4<f32>;
@group(0) @binding(1)
var<uniform> u_model_matrix: mat4x4<f32>;
@group(0) @binding(2)
var<uniform> u_camera: vec3<f32>;
@group(0) @binding(3)
var<uniform> u_light_direction: vec3<f32>;
@group(0) @binding(4)
var<uniform> u_light_color: vec3<f32>;
@group(0) @binding(5)
var<uniform> u_ambient_light_color: vec3<f32>;
@group(0) @binding(6)
var<uniform> u_ambient_light_intensity: f32;
@group(0) @binding(7)
var<uniform> u_alpha_blend: f32;
@group(0) @binding(8)
var<uniform> u_alpha_cutoff: f32;
...
out.gl_Position = u_mpv_matrix * world_position;
return out;
}
...
let n = normalize(tbn * ((2.0 * no - 1.0) * vec3<f32>(1.0, 1.0, 1.0)));
...
let base_color = textureSample(base_color_tex, base_color_sampler, in.v_UV_0) * vec4<f32>(1.0, 1.0, 1.0, 1.0);
// let base_color = base_color * in.v_Color;
let f0 = vec3<f32>(0.04);
let diffuse_color_o = base_color.rgb * (vec3<f32>(1.0) - f0);
let diffuse_color: vec3<f32> = (diffuse_color_o * (1.0 - metallic)).rgb;
